...
def compute_alignment(args, worst_choices):
    ...
    for layer in range(32):
        for idx in range(900):
            ...
            if worst_choices[idx] == [1,2] or worst_choices[idx] == [2,1]:
                'remove all similaries associated with 1 and 2'
                max_non_aligned_lasttoken = max(cs_34_lasttoken,cs_43_lasttoken)
            ...
            if worst_choices[idx] == [1,3] or worst_choices[idx] == [3,1]:
                'remove all similaries associated with 1 and 2'
                max_non_aligned_lasttoken = max(cs_24_lasttoken,cs_42_lasttoken)
                if (cs_22_lasttoken > max_non_aligned_lasttoken) & (cs_44_lasttoken > max_non_aligned_lasttoken):
                    binary_alignment_matrix_lasttoken[idx][layer] = 1
                else:
                    binary_alignment_matrix_lasttoken[idx][layer] = 0

            if worst_choices[idx] == [1,4] or worst_choices[idx] == [4,1]:
                'remove all similaries associated with 1 and 2'
                max_non_aligned_lasttoken = max(cs_23_lasttoken,cs_32_lasttoken)
                if (cs_22_lasttoken > max_non_aligned_lasttoken) & (cs_33_lasttoken > max_non_aligned_lasttoken):
                    binary_alignment_matrix_lasttoken[idx][layer] = 1
                else:
                    binary_alignment_matrix_lasttoken[idx][layer] = 0

            if worst_choices[idx] == [2,3] or worst_choices[idx] == [3,2]:
                'remove all similaries associated with 1 and 2'
                max_non_aligned_lasttoken = max(cs_14_lasttoken,cs_41_lasttoken)
                if (cs_11_lasttoken > max_non_aligned_lasttoken) & (cs_44_lasttoken > max_non_aligned_lasttoken):
                    binary_alignment_matrix_lasttoken[idx][layer] = 1
                else:
                    binary_alignment_matrix_lasttoken[idx][layer] = 0

            if worst_choices[idx] == [2,4] or worst_choices[idx] == [4,2]:
                'remove all similaries associated with 1 and 2'
                max_non_aligned_lasttoken = max(cs_13_lasttoken,cs_31_lasttoken)
                if (cs_11_lasttoken > max_non_aligned_lasttoken) & (cs_33_lasttoken > max_non_aligned_lasttoken):
                    binary_alignment_matrix_lasttoken[idx][layer] = 1
                else:
                    binary_alignment_matrix_lasttoken[idx][layer] = 0

            if worst_choices[idx] == [3,4] or worst_choices[idx] == [4,3]:
                'remove all similaries associated with 1 and 2'
                max_non_aligned_lasttoken = max(cs_12_lasttoken,cs_21_lasttoken)
...
if __name__ == '__main__':
     ...
     parser.add_argument('--save_path', type=str, default='../alignment_outputs/Llama3.1/belebele_dali_2option/')
     parser.add_argument('--embedding_path', type=str,default='/fs/nexus-scratch/kravisan/embeddings/Llama3.1/belebele_dali/')
     args = parser.parse_args()
     worst_choices_to_eliminate = load_worst_wrong_answers(args)
     print('Worst 2 choices loaded')
     
     compute_alignment(args, worst_choices_to_eliminate)