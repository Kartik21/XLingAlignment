...
def mexa(matrix):
    n = len(matrix)  # Number of samples/premise
    count = [0 for i in range(n)]
    for i in range(n):
        # Get the CS of the parallel premise
        ...
        # Check if the parallel premise (diagonal element) is strictly greater than all other elements in its row (excluding itself)
        if diag_element > max(np.delete(row, i)):
            # Check if the parallel premise (diagonal element) is strictly greater than all other elements in its column (excluding itself)
