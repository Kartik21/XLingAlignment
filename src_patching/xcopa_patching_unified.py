"""
Unified XCOPA Activation Patching Script
...
def get_ids(args):
    ...
    """
    english_results_path = args.results_path + f"xcopa_eval_{args.llm_name}_translation-{args.lang}_{args.eng_mc_key}.jsonl"
    xx_results_path = args.results_path + f"xcopa_eval_{args.llm_name}_{args.lang}_{args.xx_mc_key}.jsonl"
...
def create_prompt(dataset, split, mc_val):
    """
    Create prompts for XCOPA causal reasoning task
    ...
    for i in range(len(dataset[split])):
        ...
        prompt.append(prefix_text + 'Premise: ' + premise[i] + " \n\n\n" + mc_val[0] + choice1[i] + "\n" + mc_val[1] + choice2[i] + "\n" + "\nAnswer: ")
...
def load_hf_data(args, llm):
    ...
    # Get English data - SPECIAL: XCOPA uses translation-{lang} for English
    eng_dataset_key = 'translation-' + args.lang
    eng_dataset = load_dataset("cambridgeltl/xcopa", eng_dataset_key)
    ...
    xx_dataset = load_dataset("cambridgeltl/xcopa", args.lang)
...
def run_patching_singletoken(args):
    ...
    # Prepare control indices if needed (XCOPA has only 2 answers: 0 or 1)
    control_idx_by_answer = None
    if args.experiment_type == 'control':
        control_idx_by_answer = {
            '0': [i for i, answer in enumerate(eng_answer) if i in idx and int(answer) == 0],
    ...
    json_name = (f'{args.save_path}{args.component}/{args.patching_position}/'
                f'xcopa_{experiment_prefix}eval_{args.llm_name}_{args.patching_direction}_'
    ...
    if args.patching_direction in ['ECtoXW', 'EWtoXC']:
        for i, sample_idx in enumerate(idx):
            ...
            for layer_idx in tqdm(range(32), desc=f"Sample {i+1}/{len(idx)} (idx={sample_idx})"):
                with torch.no_grad():
                    with llm.session() as session:
                        # Extract activations from English prompt at 3 positions
                        with llm.trace(eng_prompt) as t1:
                        ...
                        # Patch into target language prompt at corresponding positions
                        with llm.trace(xx_prompt) as t2:
...
def main():
    parser = argparse.ArgumentParser(
        description='Unified XCOPA Activation Patching',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Main experiment with single token patching (layer output)
  python xcopa_patching_unified.py --experiment_type main --patching_mode singletoken \\
    --patching_position lasttoken --component layeroutput --lang zh

  # Control experiment with attention output patching
  python xcopa_patching_unified.py --experiment_type control --patching_mode singletoken \\
    --patching_position lasttoken --component attnoutput --lang zh --seed 42

  # Patching at end of option
  python xcopa_patching_unified.py --experiment_type main --patching_mode singletoken \\
    --patching_position endofoption --component layeroutput --lang zh
        """
    )
    ...
    parser.add_argument('--lang', type=str, required=True,
                       help='Target language code (e.g., zh, it, et, id)')
