...
def get_embedding_layers(text, model, tokenizer, device='cuda'):
    ...
    return sentence_embeddings_last_token
...
def main():
    ...
    parser.add_argument('--gpus', type=str, help='GPUs to use, e.g. "0".')
    ...
    parser.add_argument('--cache_dir', type=str, help='Directory for caching the model (optional).')
    parser.add_argument('--file_ext', type=str, help='File extension for input files (optional, default: .txt).')
    ...
    for language, texts in tqdm(result_dict.items()):
        ...
        for text in texts:
            ...
            embds_last_token = get_embedding_layers(text['text'], model, tokenizer)
            for layer in range(len(embds_last_token)):
